...
ThirdEye bridges the gap between visible activity and real engineering impact. ThirdEye ingests communication logs (Slack metadata/Github PR messaging) and execution data (GitHub PR's) to compute a per-team, size-stable “True Contribution” score that emphasizes quality over quantity. Signals include: complexity-weighted commits (file churn, commit size), critical-path fixes (bug severity & downstream dependency counts), feature ownership (PR-to-release traceability) and knowledge-sharing (code review depth, docs authored, mentoring events), and context-aware visibility (messages that route work vs. messages that drive decisions). A machine learning model ranks contributors only within their team using percentile/rank-based scoring and robust outlier smoothing so metrics remain comparable as team size varies.
